...
To remedy this, Petr Stehlík has an amazing project, [PARCP-USB](https://joy.sophics.cz/parcp/parcp-usb.html). I reached out to him and he was very helpful.

He builds a special adapter that enables bidirectional data transfer using the parallel port. The adapter would work fine with the IBM PC, but it uses an open source program called [PARCP](https://github.com/joysfera/parcp). PARCP was written for 32 bit processors and is compiled to DOS using [DJGPP](https://www.delorie.com/djgpp/). Petr said it would be possible to port it to 8086 DOS but with some effort.
